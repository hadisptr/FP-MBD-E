...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE Buku TO Karyawan;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE Anggota TO Karyawan;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE Transaksi TO Karyawan;

--- Role anggota
CREATE ROLE Anggota;
GRANT SELECT ON TABLE Buku TO Anggota;
GRANT SELECT ON TABLE Anggota TO Anggota;
GRANT SELECT ON TABLE Transaksi TO Anggota;


-- JOIN

--- 1. Query Pencarian Transaksi Buku Berdasarkan Nama Anggota
SELECT T.ID_Transaksi, A.Nama AS Nama_Anggota, B.Judul AS Judul_Buku
FROM Transaksi T
JOIN Anggota A ON T.ID_Anggota = A.ID_Anggota
JOIN Buku B ON T.ID_Buku = B.ID_Buku
WHERE A.Nama LIKE '%Amanda%' 

--- 2. Query Pencarian Transaksi Buku Berdasarkan Tanggal Pinjam
SELECT T.ID_Transaksi, B.Judul AS Judul_Buku, T.Tgl_Pinjam
FROM Transaksi T
JOIN Buku B ON T.ID_Buku = B.ID_Buku
WHERE T.Tgl_Pinjam BETWEEN '2017-01-01' AND '2017-05-10'
...
SELECT Nama, Alamat, COUNT(id_anggota) as "jumlah_anggota"
FROM Anggota
WHERE Alamat = 'Tokyo Dome'
GROUP BY Nama, Alamat;
...
--- 1. Membuat trigger menambahkan keterangan transaksi baru
...
--- 2. Membuat trigger menambahkan keterangan buku baru
...
CREATE OR REPLACE FUNCTION search_buku_by_judul_and_tahun(judul_buku VARCHAR, tahun_terbit_param DATE)
...
BEGIN
    ...
    AND B.Tahun_Terbit = tahun_terbit_param;
END;
$$ LANGUAGE PLPGSQL;
